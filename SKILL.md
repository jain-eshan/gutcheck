...
| empty | Two lines (see First contact), then ask what they want to check **through AskUserQuestion** when you have it, never as a wall of prose. Without that tool (claude.ai, ChatGPT), ask the same thing as one plain chat question with the same four shapes. Same four shapes as `setup.md` step 1. |
...
**Only if you have AskUserQuestion** (Claude Code). Chat hosts like claude.ai and ChatGPT don't, and there a typed question is the normal way to ask and the reply arrives on its own. In those hosts, everywhere this skill says AskUserQuestion, ask one plain question in chat, with the options as a short list, and end your turn on it. The rule below about never ending on prose applies only where the dialog exists.
